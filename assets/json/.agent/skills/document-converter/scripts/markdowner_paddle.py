...
def convert_pdf_to_markdown(pdf_path, markdown_path, force_ocr=False):
    """
    Converts a PDF file to Markdown using PaddleOCR.
    ...
    try:
        ...
        print(f"Error: PaddleOCR dependencies missing or failed to import. Details: {e}")
        print("Please install: pip install paddlepaddle paddleocr pdf2image")
        print("Note: Install 'paddlepaddle-gpu' if you have a compatible GPU.")
        return

    print(f"Converting '{pdf_path}' using PaddleOCR...")
    
    try:
        # Initialize PaddleOCR
        # use_textline_orientation=True enables orientation classification (auto-rotate)
        # lang='pt' for Portuguese/English support
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for i, page in enumerate(pages):
                ...
                if result and result[0]:
                    ...
                    # We simply extract the text parts
                    for line in result[0]:
                        text = line[1][0]
                        page_text_blocks.append(text)
                
                # Simple reconstruction: join with newlines
                # For better markdown, one might try to infer headers based on font size/position
                # but PaddleOCR main output is just the lines.
            ...
            print(f"Successfully converted '{pdf_path}' to '{markdown_path}' using PaddleOCR")
            
    except Exception as e:
        print(f"Fatal error during PaddleOCR conversion: {e}")
