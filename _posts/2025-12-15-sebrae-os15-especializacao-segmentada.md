---
layout: post
title: "Consultoria de Especialização Segmentada de MPEs (OS 15)"
...
```mermaid
graph TD
    A[Arquitetura de Atendimento a MPEs] --> B[Modelo 1: Genérico]
    A --> C[Modelo 2: Ajustado à Operação]
    A --> D[Modelo 3: Hiperespecializado / Setorial]

    B --> B1["Cursos amplos de gestão básica<br/>(Alto alcance / Baixa resolutividade percebida)"]
    C --> C1["Focado no formato do canal<br/>(Loja física vs. E-commerce vs. Serviços)"]
    D --> D1["Consultoria vertical no setor<br/>(Construção, Alimentação, Têxtil, Auto)"]
```
...
```mermaid
graph LR
    subgraph "Resistência Cultural"
        R1["Expectativa de Gratuidade"] --> R2["'Já pagamos impostos demais'"]
        R2 --> R3["Demanda por retorno parafiscal do sistema"]
    end
    
    subgraph "Adesão Condicionada ao Retorno"
        P1["Disposição Pragmática a Pagar"] --> P2["Garantia de Acompanhamento Individual"]
        P2 --> P3["Métricas de Retorno Mensurável / Skin in the Game"]
    end
```
